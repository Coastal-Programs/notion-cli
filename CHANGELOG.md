...
### CI
- **PR cross-compile no longer requires OAuth secrets** (fixes false-negative `CI Success` on fork/Dependabot PRs). The `Cross-Compile` job in `.github/workflows/ci.yml` now runs the new secret-free `make cross-compile` target instead of `make release`. GitHub does not expose repository secrets to pull requests from forks or Dependabot, so the old `make release` invocation failed `release-check` on every such PR even when the code was fine. The credential-embedding release build still runs on tag/release in `publish.yml`. The new Makefile target also forces `OAUTH_CLIENT_ID`/`OAUTH_CLIENT_SECRET` empty, so CI artifacts are never secret-bearing (previously every push uploaded binaries with the live secret embedded).
