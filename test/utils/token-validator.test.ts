...
describe("token-validator", () => {
  ...
  describe("hasValidTokenPrefix", () => {
    it("should accept secret_ prefix", () => {
      expect(hasValidTokenPrefix("secret_abc123xyz")).to.be.true;
    });

    it("should accept ntn_ prefix", () => {
      expect(hasValidTokenPrefix("ntn_abc123xyz")).to.be.true;
    });

    it("should reject tokens without valid prefix", () => {
      expect(hasValidTokenPrefix("invalid_token")).to.be.false;
      expect(hasValidTokenPrefix("abc123")).to.be.false;
      expect(hasValidTokenPrefix("")).to.be.false;
    });

    it("should export VALID_TOKEN_PREFIXES", () => {
      expect(VALID_TOKEN_PREFIXES).to.include("secret_");
      expect(VALID_TOKEN_PREFIXES).to.include("ntn_");
    });
  });
  ...
  describe("error JSON format", () => {
    it("should produce valid JSON output for automation", () => {
      ...
      try {
        ...
        expect(json.error).to.have.property(
          "code",
          NotionCLIErrorCode.TOKEN_MISSING,
        );
