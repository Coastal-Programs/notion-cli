...
describe("init command", () => {
  describe("command structure", () => {
    ...
    it("should have correct command metadata", async () => {
      const Init = await import("../../src/commands/init");
      expect(Init.default.description).to.include(
    ...
    it("should include example for interactive mode", async () => {
      const Init = await import("../../src/commands/init");
      const interactiveExample = Init.default.examples.find(
        (ex: any) =>
          ex.command.includes("notion-cli init") &&
  ...
  describe("error handling", () => {
    ...
    it("should provide helpful error when token is missing", async () => {
      ...
      const { validateNotionToken } =
        await import("../../src/utils/token-validator");
      ...
      try {
        ...
        expect((error as NotionCLIError).code).to.equal(
  ...
  describe("JSON output validation", () => {
    it("should validate JSON structure for error responses", async () => {
      const { NotionCLIError, NotionCLIErrorCode } =
        await import("../../src/errors/enhanced-errors");
  ...
  describe("error factory", () => {
    it("should create consistent TOKEN_MISSING errors", async () => {
      const { NotionCLIErrorFactory, NotionCLIErrorCode } =
        await import("../../src/errors/enhanced-errors");
    ...
    it("should include multiple suggestions in TOKEN_MISSING error", async () => {
      ...
      const hasConfigCommand = error.suggestions.some((s) =>
        s.command?.includes("config set-token"),
      );
      expect(hasConfigCommand).to.be.true;

      // Should have Unix/Mac export
      const hasUnixExport = error.suggestions.some((s) =>
        s.command?.includes("export NOTION_TOKEN="),
      );
      expect(hasUnixExport).to.be.true;

      // Should have Windows PowerShell
      const hasWindowsExport = error.suggestions.some((s) =>
        s.command?.includes("$env:NOTION_TOKEN="),
      );
      expect(hasWindowsExport).to.be.true;

      // Should have docs link
      const hasDocsLink = error.suggestions.some((s) =>
        s.link?.includes("developers.notion.com"),
      );
      expect(hasDocsLink).to.be.true;
    });
  ...
  describe("token input UX improvements", () => {
    ...
    it("should validate token prefix using hasValidTokenPrefix", async () => {
      const fs = require("fs");
      ...
      expect(source).to.include("hasValidTokenPrefix");
    });

    it("should accept both secret_ and ntn_ token prefixes", async () => {
      const fs = require("fs");
      const path = require("path");
      ...
      expect(source).to.include("ntn_");
      expect(source).to.include("secret_");
    });
