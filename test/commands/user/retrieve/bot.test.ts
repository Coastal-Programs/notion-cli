...
describe('user:retrieve:bot', () => {
  ...
  const apiMockUser = test
    .nock('https://api.notion.com', (api) => api.get('/v1/users/me').reply(200, responseOwnerUser))
  ...
  const apiMockWs = test
    .nock('https://api.notion.com', (api) => api.get('/v1/users/me').reply(200, responseOwnerWs))
    .stdout({ print: process.env.TEST_DEBUG ? true : false })
