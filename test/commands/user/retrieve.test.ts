...
describe('user:retrieve', () => {
  ...
  const apiMockPerson = test
    .nock('https://api.notion.com', (api) =>
      api.get('/v1/users/dummy-user-id').reply(200, responsePerson)
    )
  ...
  const apiMockBot = test
    .nock('https://api.notion.com', (api) =>
      api.get('/v1/users/dummy-user-id').reply(200, responseBot)
    )
    .stdout({ print: process.env.TEST_DEBUG ? true : false })
