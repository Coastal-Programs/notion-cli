...
export default class Init extends Command {
  ...
  private showWelcome() {
    ...
    this.log(
      `  ${colors.dim}1.${colors.reset} Configure your Notion integration token`,
    );
    this.log(
      `  ${colors.dim}2.${colors.reset} Test the connection to Notion API`,
    );
      ...
      if (!hasValidTokenPrefix(token)) {
        throw new NotionCLIError(
          ...
          [
            ...
            {
              description:
                "Tokens should look like: ntn_abc123... or secret_abc123...",
            },
          ],
        );
      }
      ...
      if (token.length < 20) {
        throw new NotionCLIError(
          ...
          [
            {
              description:
                "Notion integration tokens are typically 50+ characters",
            },
            {
              description:
                "Please verify you copied the complete token from Notion",
              link: "https://www.notion.so/my-integrations",
            },
            {
              description:
                "Token should look like: ntn_abc123... or secret_abc123...",
            },
          ],
        );
      }
      ...
      if (token.length < 20) {
        throw new NotionCLIError(
          ...
          [
            ...
            {
              description:
                "Token should look like: secret_abc123...(40+ more characters)",
            },
          ],
        );
      }
    ...
    try {
      ...
      while (true) {
        ...
        if (!this.isJsonMode && response.has_more) {
          ux.action.start(
            `Syncing databases (found ${databases.length} so far)`,
          );
        }
      ...
      if (!this.isJsonMode) {
        ...
        this.log("");
        this.log(
          `Synced ${databases.length} database${databases.length === 1 ? "" : "s"} in ${(syncTime / 1000).toFixed(2)}s`,
        );
        this.log("");
        ...
        if (databases.length > 0) {
          ...
          this.log(
            "Learn more: https://developers.notion.com/docs/create-a-notion-integration#give-your-integration-page-permissions",
          );
          this.log("");
        }
