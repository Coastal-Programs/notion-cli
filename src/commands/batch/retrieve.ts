...
export default class BatchRetrieve extends Command {
  ...
  static args = {
    ids: Args.string({
      required: false,
      description:
        "Comma-separated list of IDs to retrieve (or use --ids flag or stdin)",
    }),
  };
  ...
  static flags = {
    ...
    raw: Flags.boolean({
      char: "r",
      description:
        "output raw json (recommended for AI assistants - returns all fields)",
    }),
    ...tableFlags,
    ...OutputFormatFlags,
    ...AutomationFlags,
   ...
   */
  private async retrieveResource(
    id: string,
    type: string,
  ): Promise<RetrieveResult> {
    try {
      const resolvedId = await resolveNotionId(
        id,
        type === "database" ? "database" : "page",
      );
