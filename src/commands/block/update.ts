    ...
    try {
      ...
      if (flags.content) {
        try {
          ...
          const parseError = NotionCLIErrorFactory.invalidJson(flags.content, error as Error)
          if (flags.json) {
            this.log(JSON.stringify(parseError.toJSON(), null, 2))
          } else {
            this.error(parseError.toHumanString())
          }
          process.exit(1)
        }
      }
